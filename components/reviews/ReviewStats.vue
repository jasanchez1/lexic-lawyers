...
const getRatingPercentage = (rating: number): number => {
  ...
  // Get the percentage directly from the distribution object
  const percentage = props.stats.distribution[rating];
  
  // If the API doesn't provide percentages, we'll calculate them
  if (percentage === undefined) {
    // Fallback calculation if distribution doesn't have percentages
    const count = props.stats.distribution[rating.toString()] || 0;
